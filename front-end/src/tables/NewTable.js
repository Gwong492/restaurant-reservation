...
function NewTable() {
  ...
  const handleSubmit = async (event) => {
    ...
    console.log(`Submitting form - Table Name: ${table_name} Capacity: ${capacity}`);
    const abortController = new AbortController();

    if (!table_name || table_name.trim().length < 2) {
      console.warn(
        "Table Name validation failed: Must be at least 2 characters long."
      );
    ...
    if (!capacity || capacity <= 0) {
      console.warn("Capacity validation failed.");
    ...
    try {
      console.log("API request to create table:", newTable);
